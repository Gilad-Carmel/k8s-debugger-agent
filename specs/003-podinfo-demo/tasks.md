...
- [x] T001 Create `deploy/demo/` and `scripts/demo/` directories and add `.gitkeep` placeholders
- [x] T002 [P] Add `deploy/demo/00-namespace.yaml` — demo namespace with label `purpose: demo`
- [x] T003 [P] Add `deploy/demo/03-service.yaml` — ClusterIP service for podinfo on port 9898
...
- [x] T004 Add `deploy/demo/01-podinfo-v1.yaml` — v1 Deployment with `app: podinfo` labels, 32Mi memory limit, liveness + readiness probes on `/healthz` and `/readyz`
- [x] T005 [P] Add `deploy/demo/02-podinfo-v2-bad.yaml` — v2 Deployment patch: same as v1 plus `PODINFO_RUNTIME_ERROR=true` and `PODINFO_UI_MESSAGE="ERROR: database connection refused"`
- [x] T006 [P] Add `deploy/demo/04-pdb.yaml` — PodDisruptionBudget with `minAvailable: 1` targeting `app: podinfo`
- [x] T007 Add `scripts/demo/fire-webhook.sh` — constructs Alertmanager v4 JSON, computes HMAC-SHA256 via `openssl dgst`, sets `X-Alertmanager-Hmac` header, POSTs to `$AGENT_WEBHOOK_URL`, prints `correlation_id`; accepts `--scenario`, `--namespace`, `--pod`, `--agent-url`, `--hmac-secret`
- [x] T008 Add `scripts/demo/deploy.sh` — applies all `deploy/demo/*.yaml` in order, runs `kubectl rollout status deployment/podinfo -n demo --timeout=60s`, prints service URL
- [x] T009 [P] Add `scripts/demo/teardown.sh` — `kubectl delete namespace demo --ignore-not-found=true`
- [x] T010 Append `demo-deploy`, `demo-teardown`, `demo-reset` targets to `Makefile` per `plan.md §Phase 4`
...
- [x] T011 [US1] Add `scripts/demo/trigger-crash.sh` — port-forwards podinfo, sends `POST /panic`, polls until `restartCount >= 2` or state is `CrashLoopBackOff` (max 30s, 2s interval), calls `fire-webhook.sh --scenario KubePodCrashLooping`, prints `correlation_id`, cleans up port-forward
- [x] T012 [P] [US1] Append `demo-crash` Makefile target (depends on `demo-deploy`, runs `trigger-crash.sh`)
...
- [x] T015 [US3] Add `scripts/demo/trigger-oom.sh` — port-forwards podinfo, sends `GET /stress?mem=50&duration=30`, polls until pod state is `Terminated` with exit code 137 or OOMKilled event visible (max 45s, 3s interval), calls `fire-webhook.sh --scenario KubePodOOMKilled`, cleans up port-forward
- [x] T016 [P] [US3] Append `demo-oom` Makefile target
...
- [x] T019 [P] Add `tests/integration/test_demo_scenarios.sh` — smoke test: runs `make demo-deploy`, asserts pod Ready, runs `fire-webhook.sh --dry-run` (validate JSON without POSTing), runs `make demo-teardown`, asserts namespace gone; exit 0 on pass
- [x] T020 [P] Add guard to `scripts/demo/deploy.sh`: detect if `kubectl` is not in PATH or cluster is unreachable and print a clear error message (Principle III)
- [x] T021 [P] Update `deploy/dev.env` to document `AGENT_WEBHOOK_URL` and `ALERTMANAGER_HMAC_SECRET` variables with example values
- [x] T022 Validate quickstart.md against the implementation: run through `specs/003-podinfo-demo/quickstart.md` step-by-step and fix any command discrepancies
